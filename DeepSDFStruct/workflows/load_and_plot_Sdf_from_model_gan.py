...
if __name__ == "__main__":
    ...
    path = "C:/Users/camil/Desktop/Bachelorarbeit/DeepSDFStruct/DeepSDFStruct/trained_models/test_experiments/gan_test_experiment23.3"
    ...
    latent_vec = torch.tensor([[0.4]])

    deep_sdf_model = DeepSDFModel(decoder, latent_vec, device) # type: ignore
    ...
    sdf.plot_slice(origin=(0.0, 0.0, 0.0), normal=(0.0,1.0,0.0), ax = ax[0])
    sdf.set_latent_vec(torch.tensor([0.5]))
    sdf.plot_slice(origin=(0.0, 0.0, 0.0), normal=(0.0,1.0,0.0), ax = ax[1])
    sdf.set_latent_vec(torch.tensor([0.9]))
    sdf.plot_slice(origin=(0.0, 0.0, 0.0), normal=(0.0,1.0,0.0), ax = ax[2])
    ...
    sc = ax2.scatter(
        points[:, 0].numpy(),                 # x # type: ignore
        values.numpy(),                       # height # type: ignore
        points[:, 2].numpy(),                 # z # type: ignore
        cmap="viridis",
        c = values.numpy(),
        s=40
    )
    ...
    plt.savefig(path + "/decoderScatterPlot.png")
